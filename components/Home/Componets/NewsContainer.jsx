...
const NewsContainer = ({link,title,description,image,btn,author,date}) => {
  return (
    <div className="md:basis-1/3  flex flex-row md:flex-col ">
      ...
      <a href={link} target="_blank" arial-label="blog page" className="h-full">
      ...
      <div className=" basis-3/5 flex flex-col bg-white h-full px-4 py-2 md:py-6"> 
        <div className="flex flex-col gap-2 md:gap-4  self-center pb-4 md:w-[90%] ">
          ...
          <div className="flex gap-4">
            <p className="text-[12px] md:text-[13px]  leading-[14px] md:leading-[16px] font-[Poppins-Regular] font-[400] text-left  text-[#000]">{date}</p>
            <p className="text-[12px] md:text-[13px]  leading-[14px] md:leading-[16px] font-[Poppins-Regular] font-[400]  text-left  text-[#2A2A2A]">{author}</p>
          </div>
        </div>
      </div>
    </div>
