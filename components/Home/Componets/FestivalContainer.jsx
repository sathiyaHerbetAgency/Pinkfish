...
const overlayContainerMob="absolute z-1 w-[90vw] h-[394px] flex flex-col justify-between bg-gradinet-gray pb-5";

const text1="text-white text-[16px] leading-[22.66px] font-[Sofia] font-[400]";
const button="bg-[#2A2A2A] text-[12.31px] leading-[24.63px] font-[Sofia] font-[800] p-2 px-3 mt-3 mr-3 md:mr-0 rounded-md uppercase"
const FestivalContainer = ({data}) => {
  return (
    ...
    <div className="md:hidden flex flex-col ">
        <div className="w-[90vw] h-[394px]    self-center">
                <Link href={data.link} className={overlayContainerMob}>
                <div className="flex justify-end w-[100%] mr-3 md:w-[95%] ">
                    {data.sale&&<button className={button} type="button"> Tickets on sale </button>}
                </div>
                <div className="flex flex-col items-start gap-3 md:gap-5 mt-5">
                    <h3 className={textGallery1}>{data.title}</h3>
                    <div className="flex flex-col md:gap-2">
                        <div className="flex items-center gap-2">
                        <img src="./Home/Festival/Icons/date.png" className="max-w-[15px] max-h-[15px]" alt="date" />
                ...
                <img src={data.imgUrl} className="w-[90vw] h-[394px] object-cover" alt="slider-1" />
            </div>
    </div>
    </>
  )
